...
class _AddAnganWadiScreenState extends State<AddAnganWadiScreen> {
  ...
  static const String TYPE_ANGANWADI = 'AnganWadi';
  static const String TYPE_MINI_ANGANWADI = 'Mini AnganWadi';
  // Loading states
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      ...
      firstDate: DateTime.now(), // Can't schedule visits in the past
      lastDate: DateTime.now().add(
        const Duration(days: 365),
      ), // Allow scheduling up to 1 year ahead
    ...
    if (picked != null && picked != dateOfBirth) {
      setState(() {
        dateOfBirth =
            picked; // Consider renaming to 'visitDate' or 'scheduledDate'
      });
    }
  }
  ...
  Future<void> submitAnganwadiData() async {
    ...
    try {
      // 🔍 DEBUG: Log the selected type and what booleans will be set
      _logger.i("=== ANGANWADI TYPE DEBUG ===");
      _logger.i("selectedAnganWadiType value: '$selectedAnganWadiType'");
      _logger.i(
        "selectedAnganWadiType length: ${selectedAnganWadiType?.length}",
      );
      _logger.i(
        "Comparing with 'AnganWadi': ${selectedAnganWadiType == TYPE_ANGANWADI}",
      );
      _logger.i(
        "Comparing with 'Mini AnganWadi': ${selectedAnganWadiType == TYPE_MINI_ANGANWADI}",
      );

      // Calculate boolean values
      final bool isAnganwadi = selectedAnganWadiType == TYPE_ANGANWADI;
      final bool isMiniAnganwadi = selectedAnganWadiType == TYPE_MINI_ANGANWADI;

      _logger.i("anganwadi will be set to: $isAnganwadi");
      _logger.i("miniAnganwadi will be set to: $isMiniAnganwadi");
      ...
      final school = SchoolDetails(
        ...
        SchoolAddress: _addressController.text.trim(), // ✅ Added Address
        schoolType: selectedAnganWadiType, // ✅ Added Type
        ...
        anganwadi: isAnganwadi, // Use the calculated boolean
        miniAnganwadi: isMiniAnganwadi, // Use the calculated boolean
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: _buildAnganWadiOption(
                              TYPE_MINI_ANGANWADI,
                            ), // ✅ Use constant
                          ),
                          const SizedBox(width: 16),
                          Expanded(
                            child: _buildAnganWadiOption(TYPE_ANGANWADI),
                          ), // ✅ Use constant
                        ],
                      ),
                      const SizedBox(height: 30),
    ...
    return GestureDetector(
      onTap: () {
        setState(() {
          ...
          _logger.i("✅ AnganWadi type selected: '$option'");
        });
      },
      child: Container(
