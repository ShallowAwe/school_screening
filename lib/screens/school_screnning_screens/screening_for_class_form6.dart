...
class _ScreeningForClassFormSixState extends State<ScreeningForClassFormSix> {
  ...
  void initState() {
    ...
    fetchHospitals();
    _logger.i("Form Data from page 5: ${widget.previousData}");
    debugPrint("Form Data from page 5: ${widget.previousData}");
  }
  ...
  Future<void> fetchDiseasesByCategory() async {
    final url = Uri.parse(
      "https://NewAPIS.rbsknagpur.in/api/Rbsk/GetDiseaseByCategoryId?categoryId=5",
    );
    try {
      final response = await http.get(url);
      if (response.statusCode == 200) {
